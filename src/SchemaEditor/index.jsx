...
import { Modal, Icon, Tooltip, Select } from 'antd';
...
class SchemaEditor extends React.Component {
  ...
  componentDidMount() {
    const np = this.props;
    if (np.defaultValue) {
      const rows = np.defaultValue.split('\n').length;
      this.setState({
        jsonStr: np.defaultValue,
        leftRowCount: rows,
      });
    }
  }
  componentWillReceiveProps(np) {
    if (np.value !== this.props.value) {
      const { defaultValue = '' } = np;
      const rows = defaultValue.split('\n').length;
      this.setState({
        schema: np.value,
  ...
  handleJSONChange = (value) => {
    ...
    if (this.props && this.props.onChangeDefault) {
      this.props.onChangeDefault(value);
    }
  ...
  blurFx = (item, value, key) => { // 内容的修改
    const { schema } = this.state;
    setSchemaData(schema, item.path, key, value);
    this.setState({
      schema,
    });
